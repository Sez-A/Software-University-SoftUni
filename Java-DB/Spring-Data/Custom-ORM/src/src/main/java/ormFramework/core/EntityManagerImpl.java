...
public class EntityManagerImpl implements EntityManager {
    ...
    public <T> boolean alterTable(T entity) throws SQLException {
        List<String> columnsInTable = getAllColumnsInTableBy(entity);
        List<String> fieldsInEntity = getAllFieldsInEntity(entity);
        if (alreadyIsUpdated(columnsInTable, fieldsInEntity)) {
            return false;
        } else {
            String columnName = findMissingColumn(columnsInTable, fieldsInEntity);
            // TODO: 1.11.2021 г. Write alter query and add columnName in currentTable
            String tableName = getTableNameByEntity(entity);
            String columnDefinition = getColumnDefinition(entity, columnName);

            String alterAddQuery = String.format("ALTER TABLE %s ADD %s %s;"
                    , tableName, columnName, columnDefinition);
            PreparedStatement preparedStm = connection.prepareStatement(alterAddQuery);
    ...
    @Override
    public <T> void alterTableAddColumn(T entity, String columnName) throws SQLException {
        String tableName = getTableNameByEntity(entity);
        String columnDefinition = getColumnDefinition(entity, columnName);
        String alterAddQuery = String.format("ALTER TABLE %s ADD %s %s;"
                , tableName, columnName, columnDefinition);
    ...
    @Override
    public <T> void alterTableDropColumn(T entity, String columnName) throws SQLException {
        String tableName = getTableNameByEntity(entity);
        String alterDropQuery = String.format("ALTER TABLE %s DROP COLUMN %s;", tableName, columnName);

        PreparedStatement statement = connection.prepareStatement(alterDropQuery);
        statement.executeUpdate();
    }


    private <T> String getColumnDefinition(T entity, String columnName) {
        Set<Field> fields = Arrays.stream(entity.getClass().getDeclaredFields())
    ...
    private String findMissingColumn(List<String> columnsInTable, List<String> fieldsInEntity) {
        for (int i = 0; i < fieldsInEntity.size(); i++) {
            String currentColumnName = fieldsInEntity.get(i);
            boolean isExist = false;
            for (int j = 0; j < columnsInTable.size(); j++) {
                if (currentColumnName.equals(columnsInTable.get(j))) {
                    isExist = true;
                    break;
                }
            }
    ...
    private boolean alreadyIsUpdated(List<String> columnsInTable,
                                     List<String> fieldsInEntity) {

        return columnsInTable.size() == fieldsInEntity.size();
    }

    private <T> List<String> getAllFieldsInEntity(T entity) {
    ...
    private <T> List<String> getAllColumnsInTableBy(T entity) throws SQLException {
        String tableName = getTableNameByEntity(entity);
